...
export class SQLParser {
    ...
    try {
      ...
      for (const stmt of statements) {
        ...
        try {
          ...
          if (stmt.trim().toUpperCase().startsWith('CREATE PROCEDURE') || 
  ...
  extractViewDefinitions(ast) {
    ...
    if (views.length === 0 && ast._original_sql && 
        (ast._original_sql.includes('CREATE VIEW') || ast._original_sql.includes('CREATE OR REPLACE VIEW'))) {
  ...
  extractProcedureDefinitions(ast) {
    ...
    if (procedures.length === 0) {
      ...
      if (sqlString.includes('CREATE PROCEDURE') || sqlString.includes('CREATE OR REPLACE PROCEDURE')) {
  ...
  _extractSearchPathSchema(ast) {
    ...
    for (const stmt of ast) {
      if (stmt.type === 'set' && stmt.variant === 'search_path' && stmt.paths && stmt.paths.length > 0) {
        return stmt.paths[0];
      }
    }
    ...
    for (let i = 0; i < sql.length; i++) {
      ...
      if (char === '$' && !inSingleQuote && !inDoubleQuote) {
        ...
        if (!inDollarQuote) {
          ...
          if (isEndTag && i + dollarTag.length + 1 < sql.length && sql[i + dollarTag.length + 1] === '$') {
            inDollarQuote = false;
  ...
  _extractDataType(columnDef) {
    ...
    const typeName = def.dataType || 
                    (def.typeName?.names?.map(n => n.String?.str).join('.'));
  ...
  _isNullable(columnDef) {
    ...
    const hasPrimaryKeyConstraint = this._extractColumnConstraints(columnDef)
