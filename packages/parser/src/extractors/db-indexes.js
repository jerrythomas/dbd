...
export const extractIndexesFromSql = (sql, defaultSchema) => {
  ...
  const indexRegex = /CREATE\s+(UNIQUE\s+)?INDEX\s+(?:IF\s+NOT\s+EXISTS\s+)?(?:(\w+)\.)?([^\s.(]+)\s+ON\s+(?:(\w+)\.)?(\w+)\s*\(([^)]+)\)/gi;
  ...
  while ((match = indexRegex.exec(sql)) !== null) {
    ...
    const columns = columnsStr.split(',').map(colDef => {
      ...
      const colOrder = colParts.length > 1 && /^(ASC|DESC)$/i.test(colParts[1]) 
        ? colParts[1].toUpperCase() 
        : 'ASC';
