...
describe("Table Extractor - Functional API", () => {
  describe("extractTables", () => {
    it("should extract basic table definitions", () => {
      ...
      expect(idCol.constraints.some((c) => c.type === "PRIMARY KEY")).toBe(
        true,
      );
    ...
    it("should handle tables with foreign keys", () => {
      ...
      // Check foreign key
      const categoryIdCol = productsTable.columns.find(
        (c) => c.name === "category_id",
      );
      expect(categoryIdCol).toBeDefined();

      const fkConstraint = categoryIdCol.constraints.find(
  ...
  describe("extractTableSchema", () => {
    ...
    it("handles object table with schema", () => {
      expect(extractTableSchema({ table: { schema: "config" } })).toBe(
        "config",
      );
    });
