...
describe.skip('DDL Analyzer - Real World Examples', () => {
	...
	describe('Full Project Analysis', () => {
		...
		beforeAll(() => {
			const tablesConfig = ['ddl/table/config/lookups.ddl', 'ddl/table/config/lookup_values.ddl']

			const tablesStaging = ['ddl/table/staging/lookups.ddl', 'ddl/table/staging/lookup_values.ddl']
		...
		it('should identify table relationships', () => {
			...
			const lookupValues = projectSchema.tables.find(
				(t) => t.name === 'lookup_values' && t.schema === 'config'
			)
		...
		it('should extract column data types correctly', () => {
			// Find lookups table
			const lookups = projectSchema.tables.find(
				(t) => t.name === 'lookups' && t.schema === 'config'
			)
		...
		it('should detect view dependencies', () => {
			// Find genders view
			const gendersView = projectSchema.views.find(
				(v) => v.name === 'genders' && v.schema === 'config'
			)
		...
		it('should identify procedure parameters', () => {
			...
			const jsonbProc = projectSchema.procedures.find(
				(p) => p.name === 'import_jsonb_to_table' && p.schema === 'staging'
			)
		...
		it('should identify tables referenced in procedures', () => {
			...
			const importLookupsProc = projectSchema.procedures.find(
				(p) => p.name === 'import_lookups' && p.schema === 'staging'
			)
	...
	describe('Entity Relationship Map', () => {
		it('should generate a map of table relationships', () => {
			...
			const lookupRelation = relationships.find(
				(r) => r.source.includes('lookup_values') && r.target.includes('lookups')
			)
